<template>
  <button id="my-button" v-on:click="handleClick()">Load A Weapon to guess</button>
  ...
  <div v-if="all_data">
    ...
    <div  v-for="weapon in filteredGuessData" :key="weapon.id">
    ...
    <div id="info_container">
      <div id="info_line">
        <div v-for="item in items" :key="item" className="square" :id="item">
          ...
          <p v-else-if=" item === 'Rarity'&& answer_data.type !== submitted_answer.type" class="wrong" > {{ submitted_answer.type}} </p>
          ...
          <p v-else-if=" item === 'Rarity'&& answer_data.attack.raw !== submitted_answer.attack.raw" class="wrong" > {{ submitted_answer.attack.raw}} </p>
          ...
          <p v-else-if=" item === 'Rarity'&& answer_data.damageType === submitted_answer.damageType" class="wrong" > {{ submitted_answer.damageType}} </p>

          <p v-else-if=" item === 'Elderseal'&& answer_data.elderseal === submitted_answer.elderseal" class="correct" > {{ submitted_answer.elderseal}} </p>
          <p v-else-if=" item === 'Rarity'&& answer_data.elderseal === submitted_answer.elderseal" class="wrong" > {{ submitted_answer.elderseal}} </p>
        </div>
      </div>
    </div>
    <div>
